...
import {
  supabase, playSound, fmtNum, timeAgo,
  inputStyle, smallBtn,
...
// ─── STAGE (shared game canvas) ──────────────
interface StageProps {
  sprite: string; pos: { x: number; y: number };
  scale: number; hidden: boolean; flash: boolean;
  rotation: number; bubble: string | null; score: number;
  running: boolean;
}
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100%", overflow: "hidden",
      ...
      {bubble && (
        <div style={{
          position: "absolute",
          left: Math.min(pos.x + 6, 58) + "%",
          top: Math.max(pos.y - 26, 4) + "%",
          ...
          {bubble}
          <div style={{
            position: "absolute", bottom: -7, left: 10, width: 0, height: 0,
      ...
      <div style={{
        ...
        padding: "2px 8px", fontSize: 12, fontWeight: 800, color: "#fff",
      }}>⭐ {score}</div>
    </div>
  );
}

// ─── GAME PLAYER (play without editor) ───────
function GamePlayer({ game, onBack }: { game: Game; onBack: () => void }) {
  ...
  const run = async () => {
    ...
    setBubble(null); setHidden(false); setScore(0); setRotation(0);
    setFlash(false); setDone(false);
    const l: string[] = [];
    ...
    playSound("win"); push("🏁 Xong!"); setRunning(false); setDone(true);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={run} disabled={running} style={{
          ...
          color: running ? "#555" : "#000", fontWeight: 900,
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "8px 12px", borderTop: "1px solid #2a2a4a" }}>
        ...
        {log.length === 0
          ...
          : log.map((l, i) => (
            <div key={i} style={{
              fontSize: 12, padding: "2px 6px", borderRadius: 5, marginBottom: 2,
              color: i === log.length - 1 ? "#ccd" : "#6a6a8a",
              borderLeft: i === log.length - 1 ? "3px solid #4F8EF7" : "3px solid transparent",
            }}>{l}</div>
          ))}
      </div>
    </div>
...
function CommentsSheet({ game, user, onClose }: { game: Game; user: User; onClose: () => void }) {
  ...
  const send = async () => {
    ...
    setText("");
    // send notif to game owner
    if (game.user_id && game.user_id !== user.id) {
      await supabase.from("notifications").insert({
        user_id: game.user_id,
        type: "comment",
        message: username + " đã bình luận game " + game.name + ": " + text.trim().slice(0, 40),
      });
    }
  };
    ...
    <Modal onClose={onClose}>
      ...
      <div style={{ maxHeight: 300, overflowY: "auto", marginBottom: 12 }}>
        {loading ? (
          ...
          <div key={c.id} style={{
            background: "#1a1a3a", borderRadius: 12, padding: "10px 12px", marginBottom: 8,
          }}>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        ...
        <button onClick={send} disabled={!text.trim()} style={{
          background: text.trim() ? "#4F8EF7" : "#2a2a4a",
  ...
  return (
    <Modal onClose={onClose}>
      ...
      {notifs.length === 0 ? (
        ...
        <div key={n.id} style={{
          background: n.is_read ? "#1a1a3a" : "#1e1e4a",
...
// ─── EDITOR ──────────────────────────────────
function Editor({ game, user, onBack, onSave }: {
  game: Game; user: User; onBack: () => void; onSave: (g: Game) => void;
}) {
  ...
  const addBlock = (b: { id: string; label: string }, catKey: string) => {
    setBlocks(prev => [...prev, {
      id: nid.current++, blockId: b.id,
      label: b.label, color: BLOCK_CATEGORIES[catKey].color,
    }]);
    setTab("script");
  };
  ...
  const run = async () => {
    ...
    for (const b of blocks) {
      ...
      switch (b.blockId) {
        ...
        case "repeat":     push("🔁 Lặp 3 lần"); break;
        case "wait":       push("⏳ Chờ..."); await new Promise(r => setTimeout(r, 1000)); break;
        case "if_touch":   push("🤔 Kiểm tra..."); break;
        case "score_add":  setScore(s => s + 1); playSound("pop"); push("⭐ +1!"); break;
      }
    }
  ...
  if (fullscreen && tab === "preview") {
    return (
      <div style={{ position: "fixed", inset: 0, background: "#000", zIndex: 500, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ background: "#111128", padding: "6px 12px", maxHeight: 90, overflowY: "auto" }}>
          {log.slice(-3).map((l, i) => (
