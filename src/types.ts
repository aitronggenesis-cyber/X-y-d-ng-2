...
export const BLOCK_CATEGORIES: Record<string,{label:string;color:string;bg:string;blocks:{id:string;label:string}[]}> = {
  events:  { label:"⚡ Sự kiện",    color:"#FFB829", bg:"#2a1e00",
    blocks:[{id:"on_start",label:"🎮 Khi bắt đầu"},{id:"on_tap",label:"👆 Khi chạm"},{id:"on_key",label:"⌨️ Khi nhấn phím"}]},
  motion:  { label:"🏃 Di chuyển",  color:"#4F8EF7", bg:"#0d1e3a",
    blocks:[{id:"move_right",label:"➡ Đi phải 10"},{id:"move_left",label:"⬅ Đi trái 10"},{id:"jump",label:"⬆ Nhảy lên"},{id:"move_up",label:"⬆ Đi lên"},{id:"move_down",label:"⬇ Đi xuống"},{id:"spin",label:"🔄 Xoay tròn"}]},
  looks:   { label:"👀 Ngoại hình", color:"#A259FF", bg:"#1a0d3a",
    blocks:[{id:"say",label:"💬 Nói xin chào"},{id:"grow",label:"🔍 Phóng to"},{id:"shrink",label:"🔎 Thu nhỏ"},{id:"hide",label:"👻 Ẩn đi"},{id:"show",label:"👁️ Hiện ra"},{id:"flash",label:"✨ Nhấp nháy"}]},
  ...
  control: { label:"🔄 Điều khiển", color:"#FF6B6B", bg:"#2a0d0d",
    blocks:[{id:"repeat",label:"🔁 Lặp 3 lần"},{id:"wait",label:"⏳ Chờ 1 giây"},{id:"score_add",label:"⭐ +1 điểm"}]},
};
...
export const TRENDING_GAMES: Game[] = [
  ...
  {id:"t5",name:"Block Maze",   thumb:"🧩",sprite:"🐱",blocks:[],plays:540000, likes:28000,username:"@puzzle",is_public:true},
];
  ...
  try {
    ...
    if (type === "win") {
      [523,659,784,1047].forEach((freq,i) => {
        ...
        o2.connect(g2); g2.connect(ctx.destination); o2.frequency.value=freq;
